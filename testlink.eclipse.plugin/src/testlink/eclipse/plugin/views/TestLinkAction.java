...
import testlink.eclipse.plugin.handlers.ExecuteTestListener;
		...
		Boolean reportFlag)
	{
		...
		try {
			...
			exec = new ExecuteTestCases(pref.getTestLinkAPIClient(), tree.getTestPlan(),
				cases, null, "testlink.eclipse.plugin.views.ManualExecutor");
		...
		Object obj)
	{
		if ( obj instanceof ProjectTree ) {
			...
			if ( !invisibleRoot.hasChildren() ) {
				...
				visibleRoot.setPreferences(tree.getPreferences());
